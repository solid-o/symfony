    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode
            ...
            ->children()
                ...
                ->end()
                ->arrayNode('smithy')
                    ->info('Configure Smithy model generation from DTO metadata')
                    ->canBeEnabled()
                    ->children()
                        ->scalarNode('namespace')
                            ->info('Smithy namespace for the generated model')
                            ->defaultNull()
                        ->end()
                        ->scalarNode('service_name')
                            ->info('Smithy service shape name')
                            ->defaultNull()
                        ->end()
                        ->scalarNode('version')
                            ->info('Smithy service version')
                            ->defaultValue('1.0')
                        ->end()
                        ->scalarNode('output')
                            ->info('Default output path for the generated model')
                            ->defaultNull()
                        ->end()
                        ->arrayNode('dto_namespaces')
                            ->info('DTO namespaces used for Smithy generation. Defaults to solido.dto.namespaces when empty.')
                            ->scalarPrototype()->end()
                        ->end()
                        ->arrayNode('type_overrides')
                            ->info('Map PHP types or Class::member references to Smithy shape IDs')
